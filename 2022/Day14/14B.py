...
y_max = max(y for _, y in cave) + 2
x_min = min(x for x, _ in cave)
x_max = max(x for x, _ in cave)

while True:
    sand_coord = find_sand_resting_coord(sand_start[0], sand_start[1], y_max-1)
    cave[sand_coord] = "o"

    if sand_coord == sand_start:
        break
