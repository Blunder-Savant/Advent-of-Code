...
def search(opened, pressure, valve, minutes_left, elephant_turn):
    global max_pressure
    ...
    if valve not in opened:
        search(opened.union([valve]), pressure + flow_rates[valve] * minutes_left, valve, minutes_left - 1, elephant_turn)

        if not elephant_turn:
            search(set([valve]).union(opened), pressure + flow_rates[valve] * minutes_left, START_VALVE, MINUTES_REMAINING-1, True) 

    else:
        for next_valve in [v for v in valve_reduced_paths[valve] if v not in opened]:
            search(opened, pressure, next_valve, minutes_left - valve_reduced_paths[valve][next_valve], elephant_turn)
...
search(set([START_VALVE]), 0, START_VALVE, MINUTES_REMAINING-1, False)
